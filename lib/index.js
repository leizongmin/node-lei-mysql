...
MySQLPool.prototype.use = function (method, fn) {
  ...
  if (method === 'result') {
    ...
    this._filters.sql.push([(method === 'sql' ? 'all' : method), fn]);
  }
 ...
 */
MySQLPool.prototype._runFilter = function (method, args, callback) {
  method = method.trim().toLowerCase();
  var list = this._filters[method];
  debug('run filter [%s] size:%s', method, list && list.length);
  args = toArray(args);
  args.unshift(null); // 如果直接回调时，第一个参数表示是否出错
  ...
  function next (err) {
    ...
    var item = list[i++];
    if (!item) return callback.apply(null, arguments);
    if (err) return callback.apply(null, arguments);
    var args = toArray(arguments);
    args.push(next);

    if (method === 'result' || method === 'error') {
...
MySQLPool.prototype.query = function (sql, callback) {
  ...
  // SQL预处理
  me._runFilter('sql', [sql], function (err, sql) {
    if (err) {
      me._runFilter('error', arguments, callback);
    } else {
      me._pool.getConnection(function (err, conn) {
        conn.query(sql, function (err, data) {
          ...
          if (err) {
            ...
            var args = toArray(arguments);
            args.shift();
            me._runFilter('result', args, callback);
          }
        });
      });
    }
