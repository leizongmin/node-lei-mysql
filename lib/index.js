...
MySQLPool.prototype.updateTable = function (table, fields, indexes, callback) {
  ...
  me.showFields(table, function (err, oldFields) {
    ...
    me.query('SHOW INDEXES FROM ' + escapeName(table), function (err, oldIndexes) {
      ...
      // 3、都存在的不作修改
      oldIndexes = compareUpdateTableOldIndex(oldIndexes);
      var diffIndexes = [];
      var currIndexes = [];
      indexes.forEach(function (item) {
        currIndexes = currIndexes.concat(compareUpdateTableIndex(item));
      });
 ...
 * @return {Object}
 */
function compareUpdateTableIndex (data) {
 ...
 * @return {Array}
 */
function compareUpdateTableOldIndex (list) {
  var indexes = {};
  list.forEach(function (item) {
    var a = {};
    Object.keys(item).forEach(function (k) {
...
function parseUpdateTableIndex (op, data) {
  if (op === 'DROP') {
    if (data.primary) {
      ...
      return 'DROP INDEX ' + escapeName(data.key);
    }
  } else if (op === 'ADD') {
    if (Array.isArray(data.fields)) {
      var k = data.fields.map(escapeName).join(', ');
    } else {
      var k = escapeName(data.fields);
    }
    k = '(' + k + ')';
