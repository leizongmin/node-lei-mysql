...
function createModel (connection, name) {
  ...
  model.parseListOptions = function (options) {
    ...
    if (!options.skip && options.offset) options.skip = options.offset;
  ...
  model.create = function (data, callback) {
    return connection.insert(name, formatData(data), callback);
  };
  ...
  model.update = function (query, update, callback) {
    return connection.update(name, formatQuery(query), formatData(update), 'LIMIT 1',  callback);
  };
  ...
  model.updateAll = function (query, update, callback) {
    return connection.update(name, formatQuery(query), formatData(update), callback);
  };
